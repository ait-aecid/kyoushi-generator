...
test:
	@poetry run pytest --cov=src/cr_kyoushi/generator --cov-config .coveragerc --cov-report xml --cov-report term tests/ -sq

test-html:
	@poetry run pytest --cov=src/cr_kyoushi/generator --cov-config .coveragerc --cov-report html --cov-report term tests/ -sq

test-junit:
	@poetry run pytest --junitxml=report.xml --cov=src/cr_kyoushi/generator --cov-config .coveragerc --cov-report xml --cov-report term tests/ -sq
...
ifdef rule
	@poetry version $(rule)
	@sed -i -E "s/(__version__ =) \"(.+)\"/\1 \"$$(poetry version -s)\"/g" src/cr_kyoushi/generator/__init__.py
	@git add src/cr_kyoushi/generator/__init__.py
